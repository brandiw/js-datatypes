...
// const bondFilmTitles = bondFilms.forEach( (val) => {return val.title;})
...
// const toNum = bondFilms.gross.replace(  )
let grossSum = new Array
let grossSumInt = 0

const noDollarSigns = bondFilms.forEach( (val,index) => {
    grossSum.push(val[index].gross)
    grossSum[index] = Number(grossSum[index].replace(/\$|,/g, ''))
    grossSumInt += grossSum[index]
})

console.log(newArr)

const cumulativeGross = bondFilms.reduce( (acc, currentVal) =>{
